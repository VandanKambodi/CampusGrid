      ...
      <div className="w-[90%] max-w-7xl mx-auto pt-20 md:pt-24 pb-12 flex flex-col md:flex-row gap-8 items-start relative">
        ...
        <aside className={`fixed md:sticky top-0 md:top-24 left-0 h-screen md:h-auto w-64 bg-white dark:bg-[#080808] md:bg-transparent md:dark:bg-transparent z-40 border-r border-gray-200 dark:border-white/10 md:border-none transform transition-transform duration-300 ease-in-out p-6 md:p-0 pt-20 md:pt-0 overflow-y-auto ${isMobileMenuOpen ? 'translate-x-0 shadow-2xl' : '-translate-x-full md:translate-x-0'}`}>
          ...
          <div className="space-y-1">
            ...
            <Link onClick={() => setIsMobileMenuOpen(false)} to="/hub/communities" className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-sm text-xs font-extrabold transition-all ${isActive('/communities') ? 'bg-violet-600 text-white shadow-md shadow-violet-500/20' : 'hover:bg-gray-100 dark:hover:bg-white/5 text-gray-600 dark:text-gray-400'}`}>
            ...
            {user.role === 'admin' && (
              ...
              <Link onClick={() => setIsMobileMenuOpen(false)} to="/hub/admin/communities" className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-sm text-xs font-extrabold transition-all ${isActive('/admin/communities') ? 'bg-indigo-600 text-white' : 'text-indigo-600 hover:bg-indigo-50 dark:text-indigo-400 dark:hover:bg-indigo-500/10'}`}>
                <UsersRound className="w-4 h-4" /> Communities
              </Link>
